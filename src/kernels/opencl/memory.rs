...
/// A memory pool for reusing OpenCL buffers.
///
...
pub struct OpenCLMemoryPool {
    ...
    free_buffers: BTreeMap<usize, Vec<Buffer<f32>>>,
...
impl OpenCLMemoryPool {
    ...
    pub fn get_or_alloc(
        ...
        let buffer = Buffer::<f32>::builder()
            ...
            .len(bucket_elements)
            .build()
            .map_err(|e| anyhow::anyhow!("Failed to allocate OpenCL buffer: {}", e))?;
    ...
    pub fn return_buffer(&mut self, buffer: Buffer<f32>) {
    ...
    pub fn stats(&self) -> OpenCLMemoryPoolStats {
...
impl Default for OpenCLMemoryPool {
    fn default() -> Self {
        Self::new()
    }
}
...
#[derive(Debug, Clone)]
pub struct OpenCLMemoryPoolStats {
...
impl std::fmt::Display for OpenCLMemoryPoolStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "OpenCLMemoryPool: {:.2} MB allocated, {:.2} MB in use, {:.2} MB pooled, {:.1}% hit rate ({} hits, {} misses)",
...
/// Create a new OpenCL buffer with the given data.
pub fn create_buffer_with_data(queue: &Queue, data: &[f32]) -> anyhow::Result<Buffer<f32>> {
    let buffer = Buffer::<f32>::builder()
        .queue(queue.clone())
        ...
        .build()
        .map_err(|e| anyhow::anyhow!("Failed to create OpenCL buffer: {}", e))?;
    Ok(buffer)
}

/// Create a new zero-initialized OpenCL buffer.
pub fn create_buffer_zeros(queue: &Queue, len: usize) -> anyhow::Result<Buffer<f32>> {
    let zeros = vec![0.0f32; len];
    create_buffer_with_data(queue, &zeros)
}
...
/// Estimate memory required for model weights on OpenCL GPU.
...
/// Estimate memory required for KV cache on OpenCL GPU.
