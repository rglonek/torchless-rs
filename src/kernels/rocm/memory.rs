...
//!
//! This module provides GPU memory management utilities for AMD ROCm including
...
/// A memory pool for reusing GPU buffers on ROCm.
///
...
pub struct RocmMemoryPool {
    ...
    free_buffers: BTreeMap<usize, Vec<HipSlice<f32>>>,
...
impl RocmMemoryPool {
    ...
    /// is available, returns None and the caller should allocate a new one.
    pub fn try_get(&mut self, min_size: usize) -> Option<HipSlice<f32>> {
    ...
    /// Record a new allocation (not from pool).
    ...
    pub fn return_buffer(&mut self, buffer: HipSlice<f32>, len: usize) {
...
impl std::fmt::Display for MemoryPoolStats {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "ROCm MemoryPool: {:.2} MB allocated, {:.2} MB in use, {:.2} MB pooled, {:.1}% hit rate ({} hits, {} misses)",
...
impl PinnedBuffer {
    ...
    ///
    /// Note: True pinned memory requires calling hipHostMalloc.
    /// This implementation uses regular Vec as a fallback.
