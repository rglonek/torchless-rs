//! ROCm Tensor Types
//!
//! This module provides GPU tensor types that wrap HIP device memory.

use super::HipSlice;

/// A tensor stored on ROCm (HIP) device memory.
///
/// This type wraps a `HipSlice<f32>` and provides shape information.
#[derive(Clone)]
...
impl RocmTensor {
    /// Create a new 1D tensor from HIP slice.
    pub fn new_1d(data: HipSlice<f32>, len: usize) -> Self {
        Self {
            data,
            shape: vec![len],
            len,
    ...
    /// Create a new 2D tensor from HIP slice.
    pub fn new_2d(data: HipSlice<f32>, rows: usize, cols: usize) -> Self {
        let len = rows * cols;
        Self {
            data,
            shape: vec![rows, cols],
    ...
    /// Create a new 3D tensor from HIP slice.
    pub fn new_3d(data: HipSlice<f32>, d0: usize, d1: usize, d2: usize) -> Self {
        let len = d0 * d1 * d2;
        Self {
            data,
            shape: vec![d0, d1, d2],
    ...
    /// Create a new tensor with arbitrary shape from HIP slice.
    pub fn new_with_shape(data: HipSlice<f32>, shape: Vec<usize>) -> Self {
        let len = shape.iter().product();
        Self { data, shape, len }
    }

    /// Get a reference to the underlying HIP slice.
    pub fn data(&self) -> &HipSlice<f32> {
        &self.data
    }

    /// Get a mutable reference to the underlying HIP slice.
    pub fn data_mut(&mut self) -> &mut HipSlice<f32> {
        &mut self.data
    }

    /// Consume the tensor and return the underlying HIP slice.
    pub fn into_data(self) -> HipSlice<f32> {
        self.data
    }
...
// =============================================================================
// Batch of ROCm Tensors
// =============================================================================

/// A batch of tensors on ROCm device memory.
///
/// Used for batched operations like batched matrix multiplication.
#[derive(Clone)]
pub struct RocmTensorBatch {
...
impl RocmTensorBatch {
    /// Create a new batch from HIP slice.
    pub fn new(data: HipSlice<f32>, batch_size: usize, tensor_shape: Vec<usize>) -> Self {
...
impl std::fmt::Debug for RocmTensorBatch {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RocmTensorBatch")
...
impl std::fmt::Debug for RocmKVCache {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("RocmKVCache")
...
mod tests {
    #[test]
    fn test_rocm_tensor_shape() {
        // Test shape computation logic
